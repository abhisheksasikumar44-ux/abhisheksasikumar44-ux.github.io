...
window.shareToWhatsApp = async function (id) {
    ...
    try {
        ...
        // 1. Get the current app URL
        const appLink = window.location.origin + window.location.pathname;

        // 2. Fetch short link from TinyURL API (Free)
        // We will try to create a custom alias: vidyalost- + the first 6 chars of the item ID
        // If the alias is already taken, TinyURL will return a 422 error, so we fall back to a random one.
        let shortUrl = appLink; // Fallback
        const customAlias = `vidya-${isLost ? 'lost' : 'found'}-${item.id.substring(0, 6)}`;
        try {
            // First try with the custom alias
            let response = await fetch(`https://tinyurl.com/api-create.php?url=${encodeURIComponent(appLink)}&alias=${customAlias}`);

            // If the alias is taken, TinyURL returns an error. Fallback to a standard random URL.
            if (!response.ok) {
                response = await fetch(`https://tinyurl.com/api-create.php?url=${encodeURIComponent(appLink)}`);
            }
            ...
            // Will just use the long URL if it fails
        }
